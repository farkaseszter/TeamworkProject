...
format long
% --- User settings ---
filename_d = 'proppel_20_down_1460_1240_02.csv';
headerLines = 7;
rpm_d = zeros(1,12);
rpm_d(1,1) = 1460;
for rpm_var = 1:11
...
filename_u = 'proppel_20_up_1240_1460_02.csv';
rpm_u = zeros(1,12);
rpm_u(1,1) = 1240;
for rpm_var = 1:11
...
ends_u = [30 65;75 125;135 185;195 245;255 305;315 365;375 425;
    435 485;495 545;555 605;615 665;675 725];
time_diff_u = 50;
...
title(t1, 'Raw displacement vs time data - down at l=20%')
...
if plot_segment==1
    ...
    title(t2, 'Segmented displacement vs time diagrams with mean values - down at l=20%')
end
...
title(t3, 'Raw displacement vs time data - up at l=20%')
...
if plot_segment==1
    ...
    title(t4, 'Segmented displacement vs time diagrams with mean values - up at l=20%')
end
...
title(t5, 'Maximum displacement values vs rpm at l=20%')
%% 
...
for j1 = 1:2
    ...
    for j2 = 2:3
        ...
        title('Phase diagram of measurement point No.'+string(j1)+' in the x'+string(titles(j2))+'-plane - down at l=20%')
        xlim(ylims_d(j1,1,:))
        ylim(ylims_d(j1,j2,:))
        grid on;
        hold off
    ...
    title('Phase diagram of measurement point No.'+string(j1)+' in the zy-plane - down at l=20%')
    xlim(ylims_d(j1,3,:))
    ylim(ylims_d(j1,2,:))
    grid on;
    hold off
...
for j1 = 1:2
    ...
    for j2 = 2:3
        ...
        xlabel('Displacement in x-direction [mm]');
        ylabel('Displacement in '+string(titles(j2))+'-direction [mm]');
        title('Phase diagram of measurement point No.'+string(j1)+' in the x'+string(titles(j2))+'-plane - up at l=20%')
        xlim(ylims_u(j1,1,:))
        ylim(ylims_u(j1,j2,:))
        grid on;
        hold off
    ...
    xlabel('Displacement in z-direction [mm]');
    ylabel('Displacement in y-direction [mm]');
    title('Phase diagram of measurement point No.'+string(j1)+' in the zy-plane - up at l=20%')
    xlim(ylims_u(j1,3,:))
    ylim(ylims_u(j1,2,:))
    grid on;
    hold off
